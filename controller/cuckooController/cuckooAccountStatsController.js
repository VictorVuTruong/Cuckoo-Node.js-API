...
const cuckooUserCommentInteractionModel = require(`${__dirname}/../../model/cuckooModel/cuckooUserCommentInteractionModel`);

// Import the Cuckoo user profile visit model
const cuckooUserProfileVisitModel = require(`${__dirname}/../../model/cuckooModel/cuckooUserProfileVisitModel`);
...
exports.updateUserInteractionFrequency = catchAsync(
  async (request, response, next) => {
    ...
    const listOfLikesOfUser = await cuckooPostLikeModel.find({
      whoLike: userId,
    });

    // Reference the database to get list of comments by the user
    const listOfCommentsOfUser = await cuckooPostCommentModel.find({
      writer: userId,
    });
    ...
    for (i = 0; i < listOfLikesOfUser.length; i++) {
      ...
      const postObject = await cuckooPostModel.findOne({
    ...
    for (i = 0; i < arrayOfUserInteractWith.length; i++) {
      ...
      const userInteractionObjectBetweenUsers = await cuckooUserInteractionModel.findOne(
        {
          user: userId,
      ...
      if (userInteractionObjectBetweenUsers == null) {
        ...
        await cuckooUserInteractionModel.create({
          user: userId,
          interactWith: arrayOfUserInteractWith[i],
      ...
      // update it
        ...
        await cuckooUserInteractionModel.findByIdAndUpdate(
...
exports.getUserInteractionFrequency = catchAsync(
  async (request, response, next) => {
    ...
    const userInteractionObjectsOfUser = await cuckooUserInteractionModel
      .find({
        user: userId,
      })
...
exports.getInteractionStatusForUser = catchAsync(
  async (request, response, next) => {
    ...
    if (limit != 0) {
      userInteractionObjectsForUser = await cuckooUserInteractionModel
        .find({
          interactWith: userId,
        })
        .sort({ interactionFrequency: -1 })
...
exports.updateLikeStatusForUser = catchAsync(
  async (request, response, next) => {
    ...
    const listOfPostsByUser = await cuckooPostModel.find({
      writer: userId,
    });
    ...
    for (i = 0; i < listOfPostsByUser.length; i++) {
      ...
      const listOfLikesOfPost = await cuckooPostLikeModel.find({
    ...
    for (i = 0; i < arrayOfUsersInteractWithPost.length; i++) {
      ...
      const userLikeInteractionObjectBetween2Users = await cuckooUserLikeInteractionModel.findOne(
        {
          user: userId,
      ...
      if (userLikeInteractionObjectBetween2Users == null) {
        await cuckooUserLikeInteractionModel.create({
          user: userId,
          likedBy: arrayOfUsersInteractWithPost[i],
      ...
      // Otherwise, just update it
        ...
        await cuckooUserLikeInteractionModel.findByIdAndUpdate(
...
exports.getLikeInteractionStatusOfUser = catchAsync(
  async (request, response, next) => {
    ...
    if (limit != 0) {
      likeInteractionOfUser = await cuckooUserLikeInteractionModel
        .find({
          user: userId,
        })
        .sort({ numOfLikes: -1 })
...
exports.updateCommentStatusForUser = catchAsync(
  async (request, response, next) => {
    ...
    for (i = 0; i < listOfPostsByUser.length; i++) {
      ...
      const listOfCommentsOfPost = await cuckooPostCommentModel.find({
        postId: listOfPostsByUser[i]._id,
      });
    ...
    for (i = 0; i < arrayOfUsersInteractWithPost.length; i++) {
      ...
      const userCommentInteractionObjectBetween2Users = await cuckooUserCommentInteractionModel.findOne(
        {
          user: userId,
      ...
      if (userCommentInteractionObjectBetween2Users == null) {
        await cuckooUserCommentInteractionModel.create({
          user: userId,
          commentedBy: arrayOfUsersInteractWithPost[i],
      ...
      // Otherwise, just update it
        ...
        await cuckooUserCommentInteractionModel.findByIdAndUpdate(
...
exports.getCommentInteractionStatusOfUser = catchAsync(
  async (request, response, next) => {
    ...
    if (limit != 0) {
      commentInteractionOfUser = await cuckooUserCommentInteractionModel
        .find({
          user: userId,
        })
...
exports.updateUserProfileVisit = catchAsync(async (request, response, next) => {
  ...
  const userProfileVisitObjectBetween2Users = await cuckooUserProfileVisitModel.findOne(
    {
      user: visitedUserId,
  ...
  if (userProfileVisitObjectBetween2Users == null) {
    ...
    await cuckooUserProfileVisitModel.create({
      user: visitedUserId,
      visitedBy: visitorUserId,
      numOfVisits: 1,
    });
  ...
  // just update it (increase num of visit by 1)
    ...
    await cuckooUserProfileVisitModel.findByIdAndUpdate(
...
exports.getProfileVisitStatusForUser = catchAsync(
  async (request, response, next) => {
    ...
    // Based on limit to return right number of records
    if (limit != 0) {
      profileVisitObjectsOfUser = await cuckooUserProfileVisitModel
        .find({
          user: userId,
        })
        .sort({ numOfVisits: -1 })
...
exports.getBriefAccountStatsForUser = catchAsync(
  async (request, response, next) => {
    ...
    // Interaction
    const userInteractionObjectsForUser = await cuckooUserInteractionModel
      .find({
        interactWith: userId,
      })
      .sort({ interactionFrequency: -1 })
    ...
    // Like
    const likeInteractionOfUser = await cuckooUserLikeInteractionModel
      .find({
        user: userId,
      })
      .sort({ numOfLikes: -1 })
    ...
    // Comment
    const commentInteractionOfUser = await cuckooUserCommentInteractionModel
      .find({
        user: userId,
      })
      .sort({ numOfComments: -1 })
    ...
    // Profile visit
    const profileVisitObjectsOfUser = await cuckooUserProfileVisitModel
      .find({
        user: userId,
      })
      .sort({ numOfVisits: -1 })
